...
\begin{abstract}
  ...
  per-step cost and throughput across grid sizes from $128^{2}$ to
  $4096^{2}$ and find that throughput peaks at $N = 512$ at
  \SI{3.11}{Gcell/s}, holds at \SI{3.06}{Gcell/s} at $N = 1024$, and
  drops gracefully to \SI{2.51}{Gcell/s} at $N = 4096$ as the cuFFT
  algorithm transitions out of its in-kernel regime. The per-step cost
  scales as $O(N^{2} \log N)$ across more than four grid doublings, as
  predicted by the FFT complexity. An embarrassingly parallel four-GPU
  parameter sweep on a single Frontera RTX node delivers about 80\,\%
  aggregate efficiency, with a measurable contention attributable to
  shared host-side resources. A \SI{10}{\second} video of the
  ``spirals'' regime is attached to the repository
...
grid side $N$. Throughput rises sharply from
\SI{0.52}{Gcell/s} at $N = 128$ to a peak of \SI{3.11}{Gcell/s} at
$N = 512$, holds within a few percent of the peak at $N = 1024$
(\SI{3.06}{Gcell/s}), and then declines gently to \SI{2.51}{Gcell/s}
at $N = 4096$. The non-monotonic behavior is discussed below.

Figure~\ref{fig:timestep} plots the same data as cost
...
confirming the expected FFT-based asymptotics. The deviation at small
$N$ (where the measured cost is higher than the reference) is due to
fixed kernel-launch overhead becoming the dominant term in the budget.
...
of \SI{31}{\micro\second} is dominated by launch overhead and the
\num{16384} cells are insufficient to fill the 3072 CUDA cores of
the RTX 5000. At $N \geq 512$ the GPU is occupation-bound and the
curve enters the FFT-asymptotic regime.
...
$1024^{2}$ simulations concurrently. Wall time grows from
\SI{1.58}{\second} at $K = 1$ to \SI{1.96}{\second} at $K = 4$ ---
a per-process slowdown of about 24\,\% when the four GPUs are
saturated. The corresponding aggregate speedup is
$K T_{1}/T_{K} = 3.22$ at $K = 4$, i.e. about 80\,\% of the ideal
$4\times$. The two-GPU efficiency is much better, at about 91\,\%.

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{figures/multi_gpu_scaling}
  \caption{Multi-GPU embarrassingly parallel sweep on a single
  Frontera RTX node. (a) Wall time grows modestly from
  \SI{1.58}{\second} at $K = 1$ to \SI{1.96}{\second} at $K = 4$.
  (b) Effective aggregate speedup $K T_{1}/T_{K}$: 1.83$\times$ on
  two GPUs (91\,\% efficiency) and 3.22$\times$ on four GPUs
  (80\,\% efficiency), versus the linear-ideal reference.}
  \label{fig:multigpu}
\end{figure}

The deviation from the ideal $4\times$ is meaningful and worth
unpacking. The four Quadro RTX 5000 GPUs sit on independent PCIe
segments, but they share several host-side resources: a single
CPU socket runs the four user processes, the cuFFT plan-creation
path serializes on the CUDA driver, and the four processes contend
for L3 cache and DRAM bandwidth on the host while marshaling launch
arguments. Inspection of the per-process logs shows that the
\emph{steady-state} per-step time is unchanged (it remains
\SI{0.343}{\milli\second}); the slowdown is concentrated at startup,
where four cuFFT plans are built simultaneously rather than one. A
longer-running simulation would show better aggregate efficiency
than this benchmark, because the fixed startup cost gets amortized
over more steps.
...
\begin{enumerate}
  ...
  \SI{3.06}{Gcell/s} corresponds to a per-step cost of
  \SI{0.343}{\milli\second}. A conservative accounting of the memory
  traffic per step --- two R2C reads of $E$ and $I$ at \SI{4}{MB}
  each, two pointwise multiplies in the complex domain at
  approximately \SI{12}{MB} of read+write each, two C2R writes at
  \SI{4}{MB} each, and the Wilson--Cowan update at \SI{24}{MB} of
  read+write --- gives roughly \SI{80}{MB} per step, which at
  \SI{0.343}{\milli\second} corresponds to \SI{233}{GB/s} of effective
  bandwidth, or about 52\,\% of the \SI{448}{GB/s} theoretical peak.
  This is in the upper end of what has been published for cuFFT R2C
  on the RTX 5000 at these sizes~\cite{cufft}. The remaining gap to
  peak is plausibly explained by sub-peak realized bandwidth on
  individual cuFFT passes plus the un-fused pointwise kernels between
  transforms, which is where the next optimization would
  focus (see section~\ref{sec:future}).
  \item \emph{The 4-GPU parameter sweep achieves near-linear aggregate
  speedup.} Partially confirmed --- the measured aggregate speedup is
  $3.22\times$ at $K = 4$ (80\,\% efficiency), which is good but
  short of the ideal $4\times$. The slowdown is real and it grows
  with $K$. Section~\ref{sec:results} discusses the host-side
...
figure~\ref{fig:throughput} is non-monotonic. It rises steeply from
$N = 128$ to a peak at $N = 512$, then declines slowly. I had
expected the peak to be at $N = 1024$ or higher, on the assumption
that the $1024 \times 1024$ working set would best fit the GPU's
shared-memory and L1 cache budgets. The earlier-than-expected peak
is a known cuFFT phenomenon: the in-kernel single-pass FFT algorithm
operates only up to grids that fit in shared memory per
streaming-multiprocessor, and the transition to a multi-kernel
out-of-place schedule is paid for at $N = 1024$ and above. The
relative cost of that transition shrinks as $N$ grows further, which
is why the decline is gentle rather than catastrophic.
...
kernels are trivial. Measured peak throughput on a Quadro RTX 5000
is \SI{3.11}{Gcell/s} at $N = 512$ and \SI{3.06}{Gcell/s} at
$N = 1024$. The per-step cost scales as $O(N^{2} \log N)$ as
predicted. A four-GPU embarrassingly parallel parameter sweep on a
single Frontera RTX node achieves $3.22\times$ aggregate speedup, an
80\,\% efficiency that is good but lower than the trivial $4\times$
one might naively expect; the gap is explained by host-side
contention at startup. The choice of parameter-sweep multi-GPU over
domain-decomposition multi-GPU is still justified for this class of
problem at these grid sizes, because domain decomposition would have
paid PCIe latency on every time step rather than once.
